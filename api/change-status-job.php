<?php
    // http://localhost/api/change-status-job.php
    ...
    try{
        ...
        if(!isValidInput($jobId))
        {
            throw new Exception("Trạng thái công việc không được để trống!");
        }        
        ...
        if(count($job) > 0)
        {
            $query = "UPDATE job SET status_job = $status WHERE id = $jobId";
            $result  = $dbConn->query($query);
            echo json_encode(
                array(
                    "status" => true,
